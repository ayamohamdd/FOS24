...
struct Env* dequeue(struct Env_Queue* queue)
{
	...
	struct Env* envItem = LIST_FIRST(queue);
	if (envItem != NULL)
	{
...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	if(e->env_status == ENV_NEW) return; //If its tatus is NEW do nothing
	//Else
	fixed_point_t pri_max=fix_int(num_of_ready_queues-1);
	fixed_point_t fix_num4=fix_int(4);
	fixed_point_t fix_nice=fix_int(e->nice);
	fixed_point_t fix_num2=fix_int(2);
	fixed_point_t div=fix_div(e->recentCPU,fix_num4); //recentcpu/4
	fixed_point_t mul=fix_mul(fix_nice,fix_num2); //nice*2
	fixed_point_t sub1=fix_sub(pri_max,div);//max-recentcpu/4
	fixed_point_t sub2=fix_sub(sub1,mul);//max-recentcpu/4 - nice*2
	int priority=fix_trunc(sub2);
	if(priority>num_of_ready_queues-1)priority=num_of_ready_queues-1;
