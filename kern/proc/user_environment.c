...
//	return;
	...
	{
		//  Free the pages in the page working set from the main memory
		struct WorkingSetElement *wse ;
		...
		// Free the pages in the LRU lists from the main memory
			...
			//  Free the page working set array itself
//			if (&(e->page_WS_list) != NULL) {
//			     kfree((&e->page_WS_list));
//
//			}
//
//			// Free the pages LRU lists array itself
//			 if (&(e->ActiveList) != NULL) {
//			        kfree(&(e->ActiveList));
//			    }
//			 if (&(e->SecondList) != NULL) {
//			        kfree(&(e->SecondList));
//			    }

			// Free all page tables in the entire user virtual memory
			for (uint32 i = 0; i < PDX(USER_TOP); i++) {
					    // only look at mapped page tables
				if (!(e->env_page_directory[i] & PERM_PRESENT)) {
					continue;
				}
				// find the physical address of the page table
				uint32 pa = EXTRACT_ADDRESS(e->env_page_directory[i]);
				struct FrameInfo *pt_frame_info = to_frame_info(pa);
...
//				     if (ptr_page_table[pteno] & PERM_PRESENT) {
//				     uint32 pa = EXTRACT_ADDRESS(ptr_page_table[pteno]);
//				kfree((void*)pa);
			}


			// Free the page directory from the main memory
			uint32 pd_phys_addr =EXTRACT_ADDRESS(e->env_cr3);
			struct FrameInfo *pd_frame_info = to_frame_info(pd_phys_addr);
			free_frame(pd_frame_info);
