...
void* sys_get_perm(uint32 va)
{
	return (void*)pt_get_page_permissions(curenv->env_page_directory, va);
}
void* sys_hardlimit ()
{
	//syscall.c->trap line 481
...
void* sys_sbrk(int increment)
{
	...
	if(increment > 0) {
		uint32 old_sbrk = env->brk;
		env->brk = ROUNDUP(env->brk + increment, PAGE_SIZE);
		if(env->brk > env->hardlimit)
			return (void*)-1;
		...
		struct FrameInfo* ptr_frame_info;
		allocate_user_mem(curenv, ROUNDUP(old_sbrk, PAGE_SIZE), ROUNDUP(increment, PAGE_SIZE));
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	//TODO: [PROJECT'23.MS1 - #4] [2] SYSTEM CALLS - Add suitable code here
			...
			case SYS_get_perm:
				return (uint32)sys_get_perm((uint32)a1);
				break;

			case SYS_free_user_mem:

				if(a1==0 || a1>=USER_LIMIT ||(a1>=USER_LIMIT - PAGE_SIZE && a2 >= PAGE_SIZE + 10)){
