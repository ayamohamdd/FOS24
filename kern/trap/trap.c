	...
	else
	{
		if (userTrap)
		{
			...
			//your code is here
			 ...
			 int i = pt_get_page_permissions(curenv->env_page_directory, fault_va);


			 //UNMARKED IN USER HEAP
			  if (fault_va >= USER_HEAP_START && fault_va < USER_HEAP_MAX && ((PERM_MARKED) & i) != PERM_MARKED) {
			...
			if(fault_va >= USER_LIMIT) {
			 ...
			 // exit but read only
			  ...
			  if ( (PERM_PRESENT & i) && ((PERM_WRITEABLE & i) == 0)) {
				 sched_kill_env(curenv->env_id);
			 }
