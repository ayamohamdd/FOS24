...
//	panic("allocate_user_mem() is not implemented yet...!!");
	...
	for(uint32 i = virtual_address; i < virtual_address + size; i += PAGE_SIZE) {
		...
		if((e->env_page_directory[PDX(i)] & PERM_PRESENT) != PERM_PRESENT) {
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//			struct FrameInfo* ptr_frame_info = get_frame_info(e->env_page_directory, i, &ptr_page_table);
		...
		else if(ptr_frame_info->element &&ROUNDDOWN(ptr_frame_info->element->virtual_address, PAGE_SIZE) == ROUNDDOWN(i,PAGE_SIZE))
		{
			if (e->page_last_WS_element == ptr_frame_info->element)
			{
				...
				{
				  e->page_last_WS_element = LIST_NEXT(ptr_frame_info->element);
				}
			}
		...
		int perm = pt_get_page_permissions(e->env_page_directory, i);

			if((perm & (PERM_PRESENT | PERM_USER | PERM_WRITEABLE)) == (PERM_PRESENT | PERM_USER | PERM_WRITEABLE))
