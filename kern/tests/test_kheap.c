...
int test_krealloc() {
	...
	//[3] Test krealloc by passing size = 0
	{
		...
		freeFrames = sys_calculate_free_frames();
		freeDiskFrames = pf_calculate_free_frames();
		ptr_allocations[0] = krealloc(ptr_allocations[0], 0);
		...
		ptr_allocations[2] = krealloc(ptr_allocations[2], 0);
		if ((freeDiskFrames - pf_calculate_free_frames()) != 0)
			panic(
					"krealloc: Page file is changed while it's not expected to. (pages are wrongly allocated/de-allocated in PageFile)");
		if ((sys_calculate_free_frames() - freeFrames) != 256)
		...
		ptr_allocations[5] = krealloc(ptr_allocations[5], 0);
		if ((freeDiskFrames - pf_calculate_free_frames()) != 0)
			panic(
					"Page file is changed while it's not expected to. (pages are wrongly allocated/de-allocated in PageFile)");
		...
		ptr_allocations[7] = krealloc(ptr_allocations[7], 0);
		if ((freeDiskFrames - pf_calculate_free_frames()) != 0)
			panic(
					"krealloc: Page file is changed while it's not expected to. (pages are wrongly allocated/de-allocated in PageFile)");
...
//			panic("krealloc: pages in memory are not loaded correctly");
	...
	//Test krealloc: Cut & paste
	{
		...
		if ((uint32) newAddress < (KERNEL_HEAP_START))
			panic("krealloc: Wrong start address for the allocated space... ");
		if ((uint32) newAddress != ACTUAL_START + (/*14*/11 * Mega))
			panic(
...
//			panic("krealloc: pages in memory are not loaded correctly");
		...
		if ((uint32) newAddress < (KERNEL_HEAP_START))
			panic("krealloc: Wrong start address for the allocated space... ");
		if ((uint32)newAddress != ACTUAL_START + (/*14*/21 * Mega))
			panic(
					"Wrong allocation: krealloc reallocated a new address while there is a sufficient space after it (it should return same VA)");
...
//			panic("krealloc: pages in memory are not loaded correctly");
		...
		ptr_allocations[1] = krealloc(ptr_allocations[1], 0);

		ptr_allocations[10] = krealloc(NULL, 5 * Mega - kilo);
		if ((uint32) ptr_allocations[10] < (KERNEL_HEAP_START))
						panic("krealloc: Wrong start address for the allocated space... ");
					if ((uint32)ptr_allocations[10] != (uint32)ACTUAL_START + 4 * Mega)

								panic("krealloc: Wrong start address for reallocated space");

		newAddress = krealloc(ptr_allocations[3], 2 * Mega - kilo);
		if ((uint32) newAddress < (KERNEL_HEAP_START))
				panic("krealloc: Wrong start address for the allocated space... ");
			if ((uint32)newAddress != (uint32)ACTUAL_START )

						panic("krealloc: Wrong start address for reallocated space");

			newAddress = krealloc(NULL, 0);
			if(newAddress != NULL)
				panic("va = NULL and size = 0 should return NULl");
