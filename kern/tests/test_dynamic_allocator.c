...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	//[3.3] reallocate (relocate - va NULL)
	cprintf("	3.3: reallocate in different place ( relocate - NULL )\n\n") ;
	is_correct = 1;
	{
		blockIndex = 4*allocCntPerSize -1 ;
//		new_size = allocSizes[3] /*12+16B + 2KB/2*/ + allocSizes[4]/2 /*2KB/2*/ - sizeOfMetaData();
		...
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #11.1: WRONG REALLOC - it should return NULL\n");
//			panic("test_realloc_block_FF #12.0: it should return NULL.");
		}
		...
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #12.4: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
		}

	}

	//reloate - va, relocte NULL test increase

			//[3.4] reallocate (relocate - va changed)
			cprintf("	3.4: reallocate in different place ( relocate - va )\n\n") ;
			is_correct = 1;
			{
				//allocSizes[0] = 4K between[0, 199], allocSizes[2] = 1K between[400-599]
				blockIndex = 2*allocCntPerSize; //1k to 4k
				new_size = allocSizes[0] - sizeOfMetaData();
				va = realloc_block_FF(startVAs[blockIndex], new_size);

				//check return address
				if(va == NULL || (va != startVAs[0]))
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #12.1: WRONG REALLOC - it return wrong address. Expected %x, Actual %x\n", startVAs[blockIndex] ,va);
				}
				//check reallocated block size & status
				block_size = get_block_size(startVAs[0]) ;
				if (block_size != new_size + sizeOfMetaData())
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #12.2: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",new_size + sizeOfMetaData(), block_size);
				}
				block_status = is_free_block(startVAs[0]) ;
				if (block_status != 0)
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #12.3: WRONG REALLOC! block status (is_free) not equal 0 after realloc.\n");
				}
				if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex ||	*(endVAs[blockIndex]) != blockIndex)
				{
					is_correct = 0;
	...
	//[4.2] next block is free (coalesce)
			cprintf("	4.2: next block is free (coalesce)\n\n") ;
			is_correct = 1;
			{
				...
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #15.5: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
				}
			}
